...
registerEvent("org.bukkit.event.player.PlayerJoinEvent", {
  handleEvent: function (event) {
    ...
    if (joiningPlayer.hasPermission("eldin.staff")) {
      ...
      const nightVisionEffect = new PotionEffect(
      ...
      joiningPlayer.setMetadata(
        "vanished",
        new org.bukkit.metadata.FixedMetadataValue(plugin, true)
      ); //Set the player metadata to vanished
      joiningPlayer.setAllowFlight(true); //Allow flight
      joiningPlayer.setMetadata(
        "god",
        new org.bukkit.metadata.FixedMetadataValue(plugin, true)
      ); //Set the player metadata to god
      ...
      onlinePlayers.forEach((player) => {
        if (
          player.hasPermission("eldin.staff") &&
...
registerEvent("org.bukkit.event.entity.EntityTargetEvent", {
  //Stop entities from targeting vanished players
  handleEvent: function (event) {
...
registerEvent("org.bukkit.event.entity.EntityDamageEvent", {
  //Stop damage being dealt to players in god mode or vanish
  handleEvent: function (event) {
...
registerEvent("org.bukkit.event.block.BlockBreakEvent", {
  //Stop vanished players from breaking blocks
  handleEvent: function (event) {
...
registerEvent("org.bukkit.event.block.BlockPlaceEvent", {
  //Stop vanished players from placing blocks
  handleEvent: function (event) {
    if (typeof event.getPlayer === "function") {
      const player = event.getPlayer();
      if (player.hasMetadata("vanished")) {
...
registerEvent("org.bukkit.event.entity.FoodLevelChangeEvent", {
  //Stop food level changing for players in god mode
  handleEvent: function (event) {
    const entity = event.getEntity();
    if (entity.getType().toString() === "PLAYER") {
      if (entity.hasMetadata("god")) {
...
registerEvent("org.bukkit.event.player.PlayerPickupItemEvent", {
  //Stop vanished players picking up items
  handleEvent: function (event) {
    const player = event.getPlayer();
    if (player.hasMetadata("vanished")) {
      event.setCancelled(true);
...
registerEvent("org.bukkit.event.player.PlayerDropItemEvent", {
  //Stop vanished players dropping items
  handleEvent: function (event) {
    const player = event.getPlayer();
    if (player.hasMetadata("vanished")) {
      event.setCancelled(true);
...
registerEvent("org.bukkit.event.player.PlayerInteractEvent", {
  //Stop vanished players interacting with containers which play animations
  handleEvent: function (event) {
    const player = event.getPlayer();
    if (player.hasMetadata("vanished")) {
      if (event.getAction() == "RIGHT_CLICK_BLOCK") {
        ...
        if (block) {
          ...
          if (blocktype == "CHEST" || blocktype == "TRAPPED_CHEST") {
            ...
            rightBlock = true;
            if (
              block.getState().getBlockData().getType() == "LEFT" ||
              block.getState().getBlockData().getType() == "RIGHT"
            ) {
...
registerEvent("org.bukkit.event.inventory.InventoryClickEvent", {
  //Prevent movement of items in non-interactable containers
  handleEvent: function (event) {
...
registerEvent("org.bukkit.event.inventory.InventoryDragEvent", {
  //Prevent movement of items in non-interactable containers
  handleEvent: function (event) {
    const view = event.getView();
    if (view.getTitle() === "Container Preview") {
  ...
  "org.bukkit.event.server.TabCompleteEvent",
  {
    handleEvent: function (event) {
      ...
      completions.forEach((completion) => {
        if (onlinePlayerNames.has(completion)) {
          //Check that the player is a completion so that functionality is not run on other types of commands
      ...
      if (playerCompletionExists) {
        completions.forEach((completion) => {
          if (onlinePlayerNames.has(completion)) {
            //Push the completion to new completions if it is in the set
  ...
  handleEvent: function (event) {
    ...
    if (listedPlayers != null) {
      ...
      while (iterator.hasNext()) {
        const profile = iterator.next(); // Get the profile first
        const player = Bukkit.getPlayer(profile.getId()); // Call getId() method
        if (player?.hasMetadata("vanished")) {
          iterator.remove();
        }
      }
      event.setNumPlayers(listedPlayers.size());
    }
  },
});

task.repeat(0, 2, () => {
  //Refresh an action bar every 20 ticks to notify vanished players
  const onlinePlayers = Bukkit.getOnlinePlayers();
  onlinePlayers.forEach((player) => {
    if (player.hasMetadata("vanished")) {
