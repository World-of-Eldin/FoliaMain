...
addCommand("invsee", {
    onCommand: function(sender, args) {
        ...
        if (!sender.hasPermission("invsee.use") && !sender.isOp()) {
        ...
        Scheduler.run(Bukkit.getPluginManager().getPlugin("OpenJS"), function () {
            ...
            if (onlinePlayer !== null && onlinePlayer.isOnline()) {
                ...
                sender.sendMessage(ChatColor.GREEN + "Opening " + onlinePlayer.getName() + "'s inventory");
            } else {
                ...
                try {
                    ...
                    if (!OfflinePlayer.hasPlayedBefore()) {
                        sender.sendMessage(ChatColor.RED + "Player '" + targetName + "' has never joined the server.");
                        return;
                    }
                    ...
                    const playerFile = new java.io.File(playerDataFolder, playerUUID.toString() + ".dat");

                    if (!playerFile.exists()) {
                        sender.sendMessage(ChatColor.RED + "Could not find player data for '" + OfflinePlayer.getName() + "'.");
                        sender.sendMessage(ChatColor.GRAY + "Player file expected at: " + playerFile.getPath());
                        return;
                    }
                    ...
                    try {
                        ...
                        const NBTCompressedStreamTools = org.bukkit.craftbukkit.v1_21_R3.util.CraftNBTTagConfigSerializer.NBTCompressedStreamTools ||
                        ...
                        const inventory = Bukkit.createInventory(null, 54, ChatColor.GOLD + OfflinePlayer.getName() + "'s Inventory " + ChatColor.GRAY + "(Offline)");
                        ...
                        if (nbt.hasKey("Inventory")) {
                            ...
                            for (let i = 0; i < invList.size(); i++) {
                                ...
                                try {
                                    const CraftItemStack = org.bukkit.craftbukkit.v1_21_R3.inventory.CraftItemStack;
                                    const item = CraftItemStack.asCraftMirror(
                                        net.minecraft.world.item.ItemStack.parse(
                                            Bukkit.getServer().getRegistry(org.bukkit.Registry.ITEM).registryAccess(),
                                            itemCompound
                                        ).orElse(null)
                                    );
                                    ...
                                    log.warn("Could not parse item in slot " + slot + ": " + itemError);
                                }
                            }
                        }

                        sender.openInventory(inventory);
                        sender.sendMessage(ChatColor.YELLOW + "Opening " + OfflinePlayer.getName() + "'s inventory " + ChatColor.GRAY + "(Offline - Read-only)");
                        sender.sendMessage(ChatColor.GRAY + "Note: This is a snapshot from when the player last logged out.");

                    } catch (nbtError) {
                        sender.sendMessage(ChatColor.RED + "Unable to read player data (NBT parsing failed)");
                        sender.sendMessage(ChatColor.GRAY + "This player might be using a newer data format.");
                        log.error("NBT Error for " + targetName + ": " + nbtError);
                    }

                } catch (e) {
                    sender.sendMessage(ChatColor.RED + "Error loading offline player inventory.");
