...
addCommand("checkfulltime", {
  onCommand: function (sender, args) {
    ...
    sender.sendMessage(
      "§eFullTime: §f" + fullTime + " §7(total ticks since world creation)"
    );
    sender.sendMessage(
      "§eTime: §f" + currentTime + " §7(time of day, 0-24000)"
    );
    sender.sendMessage(
      "§eCalculated Day: §f" + currentDay + " §7(fullTime / 24000)"
    );
    sender.sendMessage(
      "§eTicks until next day: §f" + (24000 - (fullTime % 24000))
    );
  ...
  200,
  {
    handler: function () {
      ...
      if (currentTime >= 23800 && currentTime <= 24000) {
        ...
        var yearsMessage = " years of the Age of Revival have passed";

        if (years === 1) {
          yearsMessage = " year of the Age of Revival has passed";
        }
